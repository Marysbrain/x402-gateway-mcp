...
export const DEFAULT_EXPECTED_SIGNING_KID =
  "SUV3J4jHSPM0kbSKjh-nhZV6eM78E3rnoaonFvTyzyU";
...
export function parseManifestSignature(value: string): SignatureFields {
  const fields = new Map<string, string>();
  for (const part of value.split(";")) {
    const at = part.indexOf("=");
    if (at <= 0) throw new Error("manifest signature header is malformed");
...
export async function verifyManifestSignature(
  ...
  keyDocument: SigningKeyDocument,
  expectedKid: string,
): Promise<void> {
  if (!signatureHeader) throw new Error("gateway manifest is unsigned");
  if (!expectedKid) throw new Error("EXPECTED_SIGNING_KID must not be empty");
  const signature = parseManifestSignature(signatureHeader);
  if (signature.kid !== expectedKid) throw new Error("gateway manifest signing key is not trusted");

  const matching = keyDocument.keys?.filter((key) => key.kid === expectedKid) ?? [];
  if (matching.length !== 1) throw new Error("trusted manifest signing key is missing or duplicated");
  ...
  if (!valid) throw new Error("gateway manifest signature verification failed");
}
